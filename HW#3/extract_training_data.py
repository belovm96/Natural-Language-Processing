        ...
        if len(state.stack) >= 3:
            ...
            for i in range(1,4):
                ...
                if word == None:
                    ...
                    np.put(input_repr, [i-1], [3])
                    
                elif pos[word_ind] == 'CD':
                    
                    np.put(input_repr, [i-1], [0])
                
                elif pos[word_ind] == 'NNP':
                    
                    np.put(input_repr, [i-1], [1])
            ...
            if c == 1:
                ...
                np.put(input_repr, [1,2], [4,4])
                
            elif c == 2:
                
                np.put(input_repr, [2], [4])
                
            else:
                
                np.put(input_repr, [0,1,2], [4,4,4])
        ...
        if len(state.buffer) >= 3: 
            ...
            for i in range(1,4):
                ...
                if word == None:
                    ...
                    np.put(input_repr, [i+2], [3])
                
                elif pos[word_ind] == 'CD':
                    
                    np.put(input_repr, [i+2], [0])
                
                elif pos[word_ind] == 'NNP':
                    
                    np.put(input_repr, [i+2], [1])
            ...
            if c == 1:
                ...
                np.put(input_repr, [4,5], [4,4])
                
            elif c == 2:
                
                np.put(input_repr, [5], [4])
                
            else:
                
                np.put(input_repr, [3,4,5], [4,4,4])
    ...
    def get_output_representation(self, output_pair):  
        ...
        for tran in trans:
            for dep_rel in dep_relations:
                ...
                possible_pairs[(tran,dep_rel)] = c
                
                c += 1
                
        possible_pairs[('shift',None)] = c
        
        indx = possible_pairs[output_pair]
        
        arr = np.array(indx)
        
        repr_out = keras.utils.to_categorical(arr,num_classes = 91)
